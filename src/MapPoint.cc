...
MapPoint::MapPoint(const cv::Mat &Pos, KeyFrame *pRefKF, Map* pMap):
    ...
    mbBad(false), mfMinDistance(0), mfMaxDistance(0), mpMap(pMap),mnBAGlobalForKF(0),mpReplaced(static_cast<MapPoint*>(NULL))
{
...
{
    boost::mutex::scoped_lock lock2(mGlobalMutex);
...
{
    boost::mutex::scoped_lock lock(mMutexFeatures);
    mObservations[pKF]=idx; // this is a std::map <key frame pointer and the index(place) of the map point>
}
...
void MapPoint::ComputeDistinctiveDescriptors()
{
    ...
    if (vDescriptors.size()>6){
        //for (int i=0; i<vDescriptors.size();i++){
            //cout<<vDescriptors[i]<<endl;
        //}
...
void MapPoint::UpdateScale(float scale,const cv::Mat &grot)
{
    SetWorldPos(grot.t()*GetWorldPos()*scale);
    mfMaxDistance *= scale;
    mfMinDistance *= scale;
}
...
void MapPoint::UpdateNormalAndDepth()
{
    ...
    cv::Mat Pos;
    {
        ...
        if(mbBad)
            return;
        observations=mObservations;
        pRefKF=mpRefKF;//the first key frame that the map point created
        Pos = mWorldPos.clone();
    }
...
int MapPoint::PredictScale(const float &currentDist, FrameKTL* pF)
{
    float ratio;
    {
        boost::mutex::scoped_lock lock(mMutexPos);
