...
class VertexScale: public BaseVertex<1, Matrix<double,1,1>>
{
  public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW    
    VertexScale();
    ...
    virtual void setToOriginImpl() {
      ...
      _estimate.fill(1.0); //filling the inital value
    }

    virtual void oplusImpl(const double* update_)
    {
      Eigen::Map<const Matrix<double,1,1>> update(update_);
      _estimate += update;
    }
};
...
class EdgeNavStateDepth : public BaseUnaryEdge<1, double, VertexNavStatePVR>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW

    EdgeNavStateDepth() : BaseUnaryEdge<1, double, VertexNavStatePVR>() {}
    ...
    void SetParams(const Matrix3d& grot) {
        GravityRot = grot;
    }
...
class EdgeNavStateDepthProjected : public BaseMultiEdge<1, double> 
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW

    EdgeNavStateDepthProjected() : BaseMultiEdge<1, double>()
    {
    resize(3);// resizing the number of edges
    }
    ...
    void SetParams(const double& shi, const IMUPreintegrator& M) {
        timeshi = shi;
        mIMUPre = M;
    }
protected:
    // Gravity vector in 'world' frame
    double timeshi;
    IMUPreintegrator mIMUPre;
    
};

class EdgeScaleDepth : public BaseUnaryEdge<1, double, VertexScale>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
