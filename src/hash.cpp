...
void haloc::Hash::init(Mat desc)
{
  ...
  initProjections(1000);// 1000 should match with the number of points
  ...
  for (uint i=0; i<r_.size(); i++)
  {
    for (int n=0; n<desc.cols; n++)
    {
      ...
      for (uint m=0; m<desc.rows; m++)
        //desc_sum += r_[i][m]*desc.at<float>(m, n);// thi sis what original is
        desc_sum += r_[i][m] * (float)desc.at<uchar>(m, n);
...
float haloc::Hash::match(vector<float> hash_1, vector<float> hash_2)
{
  ...
  if (hash_1.empty() || hash_2.empty()){
    std::cerr << "haloc vectors are empty\n";
        return EXIT_FAILURE;
  }
  float sum = 0.0;
  ...
  if(isnan(sum)){
    std::cerr << "haloc score is not defined\n";
    return EXIT_FAILURE;
  }
  return sum;
}
