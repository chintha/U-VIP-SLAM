...
{
    const char* MAP_FRAME_ID = "/USLAM/World";
    ...
    mPoints.color.b= 0.2f;
    mPoints.color.g= 0.2f;
    ...
    mKeyFrames.id=1;
    mKeyFrames.type = visualization_msgs::Marker::POINTS;
    //mKeyFrames.scale.x=0.01;
    mKeyFrames.scale.x=fPointSize;
    mKeyFrames.scale.y=fPointSize;
    ...
    mCovisibilityGraph.scale.x=0.001;
    ...
    mCurrentCamera.scale.x=0.005;//0.2; 0.03
    ...
    mReferencePoints.color.g =0.0f;
    mReferencePoints.color.b =0.0f;
    mReferencePoints.color.a = 1.0;

    //Configure Lase poses
    mLposes.header.frame_id = MAP_FRAME_ID;
    mLposes.ns = POINTS_NAMESPACE;
    mLposes.id=5;
    mLposes.type = visualization_msgs::Marker::LINE_STRIP;
    mLposes.scale.x=0.04;
    mLposes.scale.y=0.04;
    mLposes.pose.orientation.w=1.0;
    mLposes.action=visualization_msgs::Marker::ADD;
    mLposes.color.r =0.0f;
    mLposes.color.g =0.0f;
    mLposes.color.b =1.0f;
    mLposes.color.a = 1.0;
...
void MapPublisher::Refresh()
{
    ...
    if(mpMap->isMapUpdated())
    {
        ...
        vector<cv::Mat> vLastPoses = mpMap->GetLastFrames();

        PublishMapPoints(vMapPoints, vRefMapPoints);   
        PublishKeyFrames(vKeyFrames);
        if(vLastPoses.size()>0){
        PublishLastFrames(vLastPoses);
        }
...
void MapPublisher::PublishKeyFrames(const vector<KeyFrame*> &vpKFs)
{
    ...
    for(size_t i=0, iend=vpKFs.size() ;i<iend; i++)
    {
        ...
        // MST
        KeyFrame* pParent = vpKFs[i]->GetPrevKeyFrame();//GetParent();
        if(pParent)
        {
...
void MapPublisher::PublishLastFrames(const vector<cv::Mat> &vLastPoses)
{
   mLposes.points.clear();
    
    for(size_t i=0, iend=vLastPoses.size()-1 ;i<iend; i++)
    {
        cv::Mat twc = vLastPoses[i];
                
        geometry_msgs::Point msgs_o;
        msgs_o.x=twc.at<float>(0);
        msgs_o.y=twc.at<float>(1);
        msgs_o.z=twc.at<float>(2);
        
        mLposes.points.push_back(msgs_o);
        
    }

    mLposes.header.stamp = ros::Time::now();
    publisher.publish(mLposes);
